 ...
 * because of the confliction to Qualcomm 404019 baseline
 */
/* DTS2011082604240 genghua 20110826 end >*/
...
/* < DTS2011082302187 caomingxing 20110823 begin */
#ifdef CONFIG_HUAWEI_KERNEL
...
/* DTS2011082302187 caomingxing 20110823 end > */
...
/*delay time 7ms*/
#define LIMIT_TIME_BETWEEN_INTERUP 7
/*delay time 0.1ms*/
#define TIME_DELAY_SCAN 10
#endif
/* DTS2011092803933  sunhonghui 20110928 end > */
...
msmsdcc_check_status(unsigned long data)
{
	...
	unsigned int status;
/* < DTS2011092803933  sunhonghui 20110928 begin */
/* avoid shake affect when insert/pull SD card */
#ifdef CONFIG_HUAWEI_KERNEL
    ...
    if (!host->plat->status) {
        ...
        if (status ^ host->oldstat) {
            ...
            if (status) 
            {
                ...
                if (duration < (LIMIT_TIME_BETWEEN_INTERUP * HZ))
                {
                    duration = (LIMIT_TIME_BETWEEN_INTERUP * HZ) - duration;
                }
                else
                {
                	/* 100 millisecond */
                    duration = TIME_DELAY_SCAN;
                }
            } 
            else
            {
...
/* < DTS2011092803933  sunhonghui 20110928 begin */
		...
		/* DTS2011102401822 liwei 20111024 end > */
        {
           if (4 == host->pdev_id )
           {
                printk("%s: can not to disable polling for slot 1 \n",__FUNCTION__);
                mmc->caps |= MMC_CAP_NEEDS_POLL;
           }
        }
#endif
...
static void msmsdcc_early_suspend(struct early_suspend *h)
{
    ...
    /* < DTS2011102102218 sunhonghui 20111023 begin */
...
#endif
    /* DTS2011102102218 sunhonghui 20111023 end > */
	spin_unlock_irqrestore(&host->lock, flags);
};
...
/* < DTS2011092803933  sunhonghui 20110928 begin */
	...
	/* DTS2011102401822 liwei 20111024 end > */
    {
        ...
        /*<	DTS2010091001367 genghua 20100910 begin	
         ...
         * DTS2010091001367 genghua 20100910 end >*/	  
        }
    }
/* DTS2011092803933  sunhonghui 20110928 end > */
#endif
/* BU5D09338 durui 20100506 end > */
	...
	if (plat->sdiowakeup_irq) {
		/*< DTS2011011600195 wangquanli 20110210 begin */
		/* solved wifi wakelock, UE can't go to sleep because 
			when irq is triggered after power on then sdio'wakelock is locked */
		//host->sdio_irq_disabled = 1; //qualcomm has fixed this bug, comment 
		/* DTS2011011600195 wangquanli 20110210 end >*/
/*<	DTS2010121801954 liyuping 20101218 begin */
//initialize wake lock here
		wake_lock_init(&host->sdio_wlock, WAKE_LOCK_SUSPEND,
				mmc_hostname(mmc));
/*  DTS2010121801954 liyuping 20101218 end  >*/
...
/* < DTS2011030904951 xuke 20110325 begin */
...
#endif
/* DTS2011030904951 xuke 20110325 end > */	
