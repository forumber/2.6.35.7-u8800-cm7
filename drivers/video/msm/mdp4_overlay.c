...
/* when overlay_is_set is true,it means overlay pipe is open */
boolean overlay_is_set = FALSE;
#endif
...
int mdp4_overlay_set(struct fb_info *info, struct mdp_overlay *req)
{
	...
	/* The mdp_hw_revison of U8800 is 1, so this condition needs to change. */
	#ifdef CONFIG_HUAWEI_KERNEL 
		else if(mdp_hw_revision == MDP4_REVISION_V2 &&
				pipe->mixer_num == MDP4_MIXER0)
		{
			overlay_is_set = TRUE;
		}
